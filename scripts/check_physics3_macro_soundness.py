#!/usr/bin/env python3
"""Validate the concrete numeric FRACTRAN macro realization for physics3."""
...
from export_physics3_deltas import REGISTER_NAMES, physics3_delta_records


def check_macro_soundness() -> tuple[list[MacroCheckRecord], dict[str, object]]:
    records, summary = physics3_delta_records()
    return check_macro_soundness_from_export(records=records, summary=summary, registers=REGISTER_NAMES, template_set="physics3")


def main() -> None:
    parser = argparse.ArgumentParser(description="Validate the concrete numeric FRACTRAN macro realization for physics3.")
    ...
    if not args.output:
        print("Physics3 macro soundness")
