#!/usr/bin/env python3
"""Export the physics15 template slice as explicit signed-delta data."""
...
def _load_agdas_bridge_module():
    ...
    module_name = "agdas_bridge_physics15_delta_export"
...
def physics15_delta_records() -> tuple[list[Physics15DeltaRecord], dict[str, object]]:
    bridge = _load_agdas_bridge_module()
    template_rules = [
        rule for rule in bridge._template_rules("physics15") if set(rule.condition) | set(rule.action) <= set(REGISTER_NAMES)
    ]

    records: list[Physics15DeltaRecord] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export the physics15 template slice as explicit signed deltas.")
