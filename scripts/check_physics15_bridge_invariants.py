#!/usr/bin/env python3
"""Check the first end-to-end bridge invariants on the physics15 slice."""
...
from check_physics15_macro_soundness import check_macro_soundness
from export_physics15_deltas import REGISTER_NAMES


def check_physics15_bridge_invariants() -> tuple[list[BridgeCoreValidatorRecord], dict[str, object]]:
    ...
    return records, {
        ...
        "scope": "bridge-local compiler correctness on the physics15 slice",
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Check bridge-local invariants on the physics15 slice.")
    parser.add_argument("--json", action="store_true", help="Emit JSON to stdout.")
    parser.add_argument("--output", type=Path, help="Optional path to write JSON output.")
    args = parser.parse_args()

    records, summary = check_physics15_bridge_invariants()
    ...
    if not args.output:
        print("Physics15 bridge invariants")
