#!/usr/bin/env python3
"""Export the physics3 template slice as explicit signed-delta data."""
...
def _load_agdas_bridge_module():
    ...
    module_name = "agdas_bridge_physics3_delta_export"
...
def physics3_delta_records() -> tuple[list[Physics3DeltaRecord], dict[str, object]]:
    bridge = _load_agdas_bridge_module()
    template_rules = [
        rule for rule in bridge._template_rules("physics3") if set(rule.condition) | set(rule.action) <= set(REGISTER_NAMES)
    ]

    records: list[Physics3DeltaRecord] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export the physics3 template slice as explicit signed deltas.")
    ...
    records, summary = physics3_delta_records()
