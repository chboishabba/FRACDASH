#!/usr/bin/env python3
"""Dedicated 8-register carrier for the physics-local memory branch."""
...
REGISTERS: tuple[RegisterSpec, ...] = (
    ...
    RegisterSpec("R7", 227, 229, 233, "boundary return memory"),
    RegisterSpec("R8", 239, 241, 251, "transport/debt memory"),
)
...
def sample_states() -> list[dict[str, object]]:
    samples = [
        {
            "name": "carrier8_idle",
            "state": {"R1": 0, "R2": 0, "R3": 0, "R4": 1, "R5": 0, "R6": -1, "R7": 0, "R8": 0},
        },
        {
            "name": "carrier8_boundary_discharge_memory",
            "state": {"R1": 0, "R2": 0, "R3": 0, "R4": -1, "R5": 0, "R6": 0, "R7": 1, "R8": 0},
        },
        {
            "name": "carrier8_shell_debt",
            "state": {"R1": 1, "R2": 0, "R3": 1, "R4": 0, "R5": 1, "R6": 0, "R7": 0, "R8": 1},
        },
        {
            "name": "carrier8_transport_handoff",
            "state": {"R1": -1, "R2": 1, "R3": 0, "R4": -1, "R5": 0, "R6": 0, "R7": 1, "R8": -1},
        },
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate the 8-register physics-local carrier.")
    ...
    print("Physics-local 8-register carrier")
