...
- Carrier8 subset-bottleneck follow-up on `2026-03-23`: after probing the bounded 512-state analysis subset, the next branch targeted the actual sampled boundary bottlenecks (`carrier8_physics11_boundary_discharge` and boundary `carrier8_physics2_join_*` states with positive return memory). The resulting `carrier8_physics5` branch is the first real 8-register successor candidate on the shared summary surface: `boundary_to_interior` rises from `0 -> 6`, `action_rank` strict decrease improves from `0.5159 -> 0.5253`, and geodesic-like near-min ratio improves slightly, but curvature spread drops from `0.79661 -> 0.76341`. Current read: this is real progress, but not yet a clean handoff from `carrier8_physics2`.
